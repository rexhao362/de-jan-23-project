resource "aws_lambda_function" "ingestion_lambda" {
  ...
  handler       = "${local.ingestion_module_name}.${local.ingestion_lambda_name}"
  runtime       = "python3.9"
...
resource "aws_lambda_function" "process_lambda" {
  ...
  handler       = "${local.process_module_name}.${local.process_lambda_name}"
  runtime       = "python3.9"
  # AWSSDKPandas-Python39 x86_64
  layers           = ["arn:aws:lambda:us-east-1:336392948345:layer:AWSSDKPandas-Python39:5"]
  timeout          = 240
...
resource "aws_lambda_function" "load_lambda" {
  ...
  handler = "${local.load_module_name}.${local.load_lambda_name}"
  runtime = "python3.9"
  # AWSSDKPandas-Python39 x86_64
  layers           = ["arn:aws:lambda:us-east-1:336392948345:layer:AWSSDKPandas-Python39:5"]
  timeout          = 240
