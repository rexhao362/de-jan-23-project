...
class _SecretsManager:
    ...
    def get_secret_value(secret_name, default_value=None):
        ...
        Returns:
            ...
            If not defined and no default_value specified,
            returns None, otherwise default_value.
        """
        _invalid_result = None
        if not isinstance(secret_name, str):
            return _invalid_result

        secret_value = default_value if isinstance(
            default_value, str) else _invalid_result
    ...
    def get_secret_int_value(secret_name, default_value=None):
        ...
        """
        if not isinstance(secret_name, str):
            msg = f"""secret_name should be a string
            (got {secret_name} ({ type(secret_name) })"""
            raise TypeError(msg)
        ...
        if isinstance(secret_value, str):
            ...
            if isinstance(default_value, int):
                ...
                raise ValueError(
                    f"""unable to retrieve int value
                    associated with secret '{secret_name}'""")
    ...
    def get_secret_database_config(database_config_decription=None):
        ...
        Returns:
            ...
            or None if unseccessful (no description provided,
            secret_name does not exist, config is incomplete)
        """
        if not database_config_decription:
            return None

        if is_production_environ():
            config_json = _SecretsManager.get_secret_value(
                database_config_decription["name"])
            ...
            try:
                ...
                config = {
                    "credentials": {
                        ...
                        "host": environ.get(vars["host"]["name"],
                                            vars["host"]["default"]),
                        "port": int(environ.get(vars["port"]["name"],
                                                vars["port"]["default"])),
    ...
    def get_secret_totesys_db_config():
        return _SecretsManager.get_secret_database_config(
            project_secrets["totesys_database_config"]
        )

    @staticmethod
    def get_secret_warehouse_db_config():
        return _SecretsManager.get_secret_database_config(
            project_secrets["warehouse_database_config"]
        )
