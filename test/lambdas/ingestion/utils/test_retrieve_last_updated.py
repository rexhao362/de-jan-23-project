...
def test_retrieve_last_updated_function_retrieves_last_updated(bucket, s3):
    with open('./local/aws/s3/ingestion/date/last_updated.json', 'w') as f:
        test_date = {"last_updated": "2000-11-03T14:20:49.962000"}
        f.write(json.dumps(test_date))

    with open('./local/aws/s3/ingestion/date/last_updated.json', 'rb') as f:
        s3.put_object(
            Body=f,
...
def test_retrieve_last_updated_returns_default_date_if_last_updated_file_not_in_bucket(s3):
    ...
    list_files = [{'Key': item['Key']} for item in response['Contents']]
    s3.delete_objects(Bucket=get_ingested_bucket_name(), Delete={'Objects': list_files})
