from src.lambdas.process import (load_file_from_local, process, build_dim_currency)
...
def test_returned_dataframe_has_no_null_values():
    for header in dim_currency:
        for entry in dim_currency[header]:
            assert entry

def test_returned_data_is_maintained_through_build():
    assert dim_currency['currency_id'][0] == 1
    assert dim_currency['currency_code'][0] == 'code_1'
    assert dim_currency['currency_name'][0] == 'British Pounds'
